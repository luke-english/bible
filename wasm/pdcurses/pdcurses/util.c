...
# ifdef PDC_FORCE_UTF8
    ...
    // I know that WASM is little-endian, 
    // so the first byte in the WASM hexadecimal representation ( d2 ) 
    // corresponds to the last byte in normal binary notation. 
    // The binary representation of 1234 in 32 bits is 00000000 00000000 00000100 11010010 
    ...
    if (key & 0x80)
    {
        if ((key & 0xf0) == 0xf0)
        {
            if (3 < n)
            {
                key = ((string[0] & 0b00000111) << (8+8+8+2)) // www
                    | ((string[1] & 0b00110000) << (8+8+8-4)) // zz
                    | ((string[1] & 0b00001111) << (8+8+4)) // zzzz
                    | ((string[2] & 0b00111100) << (8+8-2)) // yyyy
                    | ((string[2] & 0b00000011) << (8+6)) // yy 
                    | ((string[3] & 0b00111111) << (8+0)) // xxxxxx 
                ;
